...
module.exports = {
    login: async (req, res) => {
        ...
        if (users[0].ativo && users[0].codTipoUsuario == 1) {
            ...
            res.json({ success: false, message: "Não é possível entrar com acesso MASTER", type: 2 })
        } else if (users[0].ativo && users[0].codTipoUsuario == 3) {
            res.json({ success: false, message: "Não é possível entrar com acesso ANUNCIANTE", data: users[0], type: 3 })
        } else {
